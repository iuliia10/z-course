...
function input(x) {
    ...
    if(document.getElementById('decimalVar').value == 0)
    {
     ...
     if(decimalCount == 1){
        x *= 1/10; //We are ising math to place the decimal point.
        y += x;
...
function decimalPoint(){
    if(document.getElementById('decimalVar').value == 1) {// this prevents multiople decimal points.
       document.getElementById('decimalVar').value = 1;
    }
    if(parseInt(document.getElementById('operation').value)){
        document.getElementById('result').value = 0; //if this is an empty string,it will return false.
    }
}
